<x-app-layout>
    ...
    <div class="p-6">
            ...
            <div>
                @if(auth()->user()->role === 'student')
                    @if(auth()->user()->courses->contains('course_id', $course->course_id))
                        <span class="bg-green-100 text-green-700 px-4 py-2 rounded-lg font-semibold shadow-sm">
                            ✓ Sudah Enroll
                        </span>
                    @else
                        <form action="{{ route('enroll.course', $course->course_id) }}" method="POST">
                            @csrf
                            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg transition duration-300 shadow-sm">
                                Enroll Sekarang
                            </button>
                        </form>
                    @endif
        ...
        @php
            $isEnrolled = auth()->user()->role === 'instructor' || auth()->user()->courses->contains('course_id', $course->course_id);
        @endphp
