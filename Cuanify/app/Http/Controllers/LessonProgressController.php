    ...
    public function complete($lessonId)
    {
        ...
        if ($lesson->has_quiz && $lesson->quiz) {
            ...
            if (
                ...
                return back()->with(
                    'error',
                    'Anda harus lulus quiz terlebih dahulu sebelum menyelesaikan lesson ini.'
                );
            }
        }
        ...
        if (!$existingProgress || !$existingProgress->is_completed) {
            ...
            $totalMateri = \App\Models\Lesson::where('course_id', $course->course_id)
                ->where('is_published', true)
                ->count();
                
            $enrolledLessonIds = \App\Models\Lesson::where('course_id', $course->course_id)
                ->where('is_published', true)
                ->pluck('lesson_id');
